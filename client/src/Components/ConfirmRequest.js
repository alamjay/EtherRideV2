...
class ConfirmRequest extends React.Component {
  state = {
    executeOrder: false,
    request: this.props.request.confirmRequest,
  ...
  componentDidUpdate() {
    if (this.state.executeOrder) {
      ...
      // calling the confirm request method from the smart contract
      this.props.drizzle.contracts.Vehicleshare.methods.confirmRequest.cacheSend(
        this.props.request.owner
      );

      // saving the data from the contract to the local storage
      let notifications = JSON.parse(localStorage.getItem("notifyDriver"));
      notifications = notifications.filter(
        notification => notification.rentalId !== this.props.request.rentalId
      );
              ...
              <Grid container spacing={24}>
                ...
                <Grid item xs={3}>
                  <Typography variant="title">
                    {this.props.request.make} {this.props.request.model}
                  </Typography>
